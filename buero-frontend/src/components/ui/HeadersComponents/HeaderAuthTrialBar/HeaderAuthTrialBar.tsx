...
const HeaderAuthTrialBar: React.FC<HeaderNavAuthTrialProps> = ({ isLight, from="", className="" }) => {
    return (
        <div className={["flex gap-2", className].join(" ")}>
            <Button variant={isLight ? 'outline' : 'outlineDark'} aria-label="Sign in" className={from ? "w-full" : ""}>
                Sign in
            </Button>
