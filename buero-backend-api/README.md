...
| Progress         | `/api/progress`, `/api/courses/:courseId/progress` | Загальний прогрес (`GET /me`), прогрес по курсу (`GET /courses/:id/progress`), позначити матеріал пройденим (`POST .../materials/:id/complete`), рекомендований наступний курс (`GET /recommended-next`). userId тимчасово: query/body/x-user-id. |
| Quiz             | `/api/quiz/attempts` | Почати спробу (`POST`), відправити відповідь (`POST /:attemptId/answers`), завершити (`POST /:attemptId/complete`), стан спроби для resume (`GET /:attemptId`). Покрокове збереження в answers_snapshot. |
